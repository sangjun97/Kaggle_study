## B_BASE
============================================================
# ?꾩껜 ?됯? (n=246)
============================================================
 MAE :  37.17
 RMSE :  49.41
 Bias :  -9.55 (?덉륫-?ㅼ륫, +硫?怨쇰??덉륫)
 Corr :  0.521

 [BP/?ы솕 ?꾩튂] n= 106 MAE= 42.75
 [?쇰컲 ?꾩튂] n= 140 MAE= 32.94

 [CUH 援ш컙蹂?MAE]
     0~50 : n=  59 MAE= 33.80
   50~100 : n=  81 MAE= 32.31
  100~150 : n= 106 MAE= 42.75

============================================================
# ?댁꽍 媛?대뱶
============================================================
 쨌 CUH 5?⑥쐞 ?묒옄???곗씠????MAE ?쒓퀎 ?섑븳 ~12-15 ?섏?
 쨌 Mid OI t200 MODEL MAE ~25.7 ??鍮꾧탳 湲곗?
 쨌 ?꾩옱 MAE 37.2 ??Mid ?鍮??믪쓬, ?먭? ?꾩슂
 쨌 Bias ?щ㈃(짹10?? 怨꾪넻 ?명뼢 ??蹂??target ?ъ젏寃

============================================================
# ?ы솕(?ㅼ륫150) ?꾩튂 吏꾨떒 (n=106)
============================================================
 ?ㅼ륫 150 ?꾩튂??cuh_prev(湲곗?):
 prev ?됯퇏: 104.33 (150 ?대㈃ delta 0 ?쇰줈 留욎땄, ??쑝硫?delta 媛 ?먰봽 ?꾩슂)
 prev <100 鍮꾩쑉:  39.6% (?믪쓣?섎줉 delta 濡??ы솕 紐??곕씪媛?
 ?덉륫 ?됯퇏: 107.25 (150 ??媛源뚯썙???뺤긽)
 ?덉륫 MAE :  42.75

 ?좑툘 prev<100 ?몃뜲 ?ㅼ륫 ?ы솕(150) ???꾩튂 42嫄?
 ???꾩튂???덉륫 ?됯퇏 79.1 ??delta 紐⑤뜽??89 ?먰봽瑜?紐?留뚮뱾??怨쇱냼?덉륫

============================================================
# 怨쇰??덉륫 吏꾨떒 ???ㅼ륫 ??쓬(??0) ?꾩튂 (n=45)
============================================================
 ?ㅼ륫 ?됯퇏:  16.67
 ?덉륫 ?됯퇏:  50.76 (?ㅼ륫蹂대떎 ?믪쑝硫?怨쇰??덉륫)
 Bias : +34.10
 MAE :  37.34
 cuh_prev ?됯퇏:  60.91 (?믪쑝硫?delta 濡?紐??대젮??寃?
 prev >=100 鍮꾩쑉:  17.8% (?ㅼ륫 ?????prev ?ы솕硫?湲됰씫 紐??곕씪媛?

 ?좑툘 ?ㅼ륫 ??쓬(??0) & prev??00 ???꾩튂 8嫄?
 prev ?됯퇏 125 ???ㅼ륫 18 湲됰씫?몃뜲 ?덉륫 95 (delta 媛 ?섎씫 紐??곕씪媛 怨쇰??덉륫)

## C1_WIDECLIP

============================================================
# ?꾩껜 ?됯? (n=131)
============================================================
 MAE :  37.89
 RMSE :  50.18
 Bias : -13.48 (?덉륫-?ㅼ륫, +硫?怨쇰??덉륫)
 Corr :  0.535

 [BP/?ы솕 ?꾩튂] n=  58 MAE= 44.48
 [?쇰컲 ?꾩튂] n=  73 MAE= 32.65

 [CUH 援ш컙蹂?MAE]
     0~50 : n=  35 MAE= 30.67
   50~100 : n=  38 MAE= 34.47
  100~150 : n=  58 MAE= 44.48

============================================================
# ?댁꽍 媛?대뱶
============================================================
 쨌 CUH 5?⑥쐞 ?묒옄???곗씠????MAE ?쒓퀎 ?섑븳 ~12-15 ?섏?
 쨌 Mid OI t200 MODEL MAE ~25.7 ??鍮꾧탳 湲곗?
 쨌 ?꾩옱 MAE 37.9 ??Mid ?鍮??믪쓬, ?먭? ?꾩슂
 쨌 Bias ?щ㈃(짹10?? 怨꾪넻 ?명뼢 ??蹂??target ?ъ젏寃

============================================================
# ?ы솕(?ㅼ륫150) ?꾩튂 吏꾨떒 (n=58)
============================================================
 ?ㅼ륫 150 ?꾩튂??cuh_prev(湲곗?):
 prev ?됯퇏: 110.07 (150 ?대㈃ delta 0 ?쇰줈 留욎땄, ??쑝硫?delta 媛 ?먰봽 ?꾩슂)
 prev <100 鍮꾩쑉:  39.7% (?믪쓣?섎줉 delta 濡??ы솕 紐??곕씪媛?
 ?덉륫 ?됯퇏: 105.52 (150 ??媛源뚯썙???뺤긽)
 ?덉륫 MAE :  44.48

 ?좑툘 prev<100 ?몃뜲 ?ㅼ륫 ?ы솕(150) ???꾩튂 23嫄?
 ???꾩튂???덉륫 ?됯퇏 78.9 ??delta 紐⑤뜽??82 ?먰봽瑜?紐?留뚮뱾??怨쇱냼?덉륫

============================================================
# 怨쇰??덉륫 吏꾨떒 ???ㅼ륫 ??쓬(??0) ?꾩튂 (n=23)
============================================================
 ?ㅼ륫 ?됯퇏:  18.26
 ?덉륫 ?됯퇏:  49.41 (?ㅼ륫蹂대떎 ?믪쑝硫?怨쇰??덉륫)
 Bias : +31.15
 MAE :  36.17
 cuh_prev ?됯퇏:  62.79 (?믪쑝硫?delta 濡?紐??대젮??寃?
 prev >=100 鍮꾩쑉:  21.7% (?ㅼ륫 ?????prev ?ы솕硫?湲됰씫 紐??곕씪媛?

 ?좑툘 ?ㅼ륫 ??쓬(??0) & prev??00 ???꾩튂 5嫄?
 prev ?됯퇏 125 ???ㅼ륫 20 湲됰씫?몃뜲 ?덉륫 88 (delta 媛 ?섎씫 紐??곕씪媛 怨쇰??덉륫)

## C2PW3

============================================================
# ?꾩껜 ?됯? (n=246)
============================================================
 MAE :  37.17
 RMSE :  49.41
 Bias :  -9.55 (?덉륫-?ㅼ륫, +硫?怨쇰??덉륫)
 Corr :  0.521

 [BP/?ы솕 ?꾩튂] n= 106 MAE= 42.75
 [?쇰컲 ?꾩튂] n= 140 MAE= 32.94

 [CUH 援ш컙蹂?MAE]
     0~50 : n=  59 MAE= 33.80
   50~100 : n=  81 MAE= 32.31
  100~150 : n= 106 MAE= 42.75

============================================================
# ?댁꽍 媛?대뱶
============================================================
 쨌 CUH 5?⑥쐞 ?묒옄???곗씠????MAE ?쒓퀎 ?섑븳 ~12-15 ?섏?
 쨌 Mid OI t200 MODEL MAE ~25.7 ??鍮꾧탳 湲곗?
 쨌 ?꾩옱 MAE 37.2 ??Mid ?鍮??믪쓬, ?먭? ?꾩슂
 쨌 Bias ?щ㈃(짹10?? 怨꾪넻 ?명뼢 ??蹂??target ?ъ젏寃

============================================================
# ?ы솕(?ㅼ륫150) ?꾩튂 吏꾨떒 (n=106)
============================================================
 ?ㅼ륫 150 ?꾩튂??cuh_prev(湲곗?):
 prev ?됯퇏: 104.33 (150 ?대㈃ delta 0 ?쇰줈 留욎땄, ??쑝硫?delta 媛 ?먰봽 ?꾩슂)
 prev <100 鍮꾩쑉:  39.6% (?믪쓣?섎줉 delta 濡??ы솕 紐??곕씪媛?
 ?덉륫 ?됯퇏: 107.25 (150 ??媛源뚯썙???뺤긽)
 ?덉륫 MAE :  42.75

 ?좑툘 prev<100 ?몃뜲 ?ㅼ륫 ?ы솕(150) ???꾩튂 42嫄?
 ???꾩튂???덉륫 ?됯퇏 79.1 ??delta 紐⑤뜽??89 ?먰봽瑜?紐?留뚮뱾??怨쇱냼?덉륫

============================================================
# 怨쇰??덉륫 吏꾨떒 ???ㅼ륫 ??쓬(??0) ?꾩튂 (n=45)
============================================================
 ?ㅼ륫 ?됯퇏:  16.67
 ?덉륫 ?됯퇏:  50.76 (?ㅼ륫蹂대떎 ?믪쑝硫?怨쇰??덉륫)
 Bias : +34.10
 MAE :  37.34
 cuh_prev ?됯퇏:  60.91 (?믪쑝硫?delta 濡?紐??대젮??寃?
 prev >=100 鍮꾩쑉:  17.8% (?ㅼ륫 ?????prev ?ы솕硫?湲됰씫 紐??곕씪媛?

 ?좑툘 ?ㅼ륫 ??쓬(??0) & prev??00 ???꾩튂 8嫄?
 prev ?됯퇏 125 ???ㅼ륫 18 湲됰씫?몃뜲 ?덉륫 95 (delta 媛 ?섎씫 紐??곕씪媛 怨쇰??덉륫)

## C3PW10
